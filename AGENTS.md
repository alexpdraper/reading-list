...
**The fix (`migrations.ts`):** `migrateLegacyItems()` works when only a few items' worth of space is free. A bucket write that doesn't fit is retried with half as many items. Legacy keys whose items are already verified in a bucket are removed in one batch whenever the next write doesn't fit, not after every bucket, which keeps the write count low for Chrome's per-minute rate limit. Only if not even a single item fits anywhere does it fall back to *swapping* one item: remove its legacy key, write it to its bucket, and put the key back if that write fails. The item is also in the local backup throughout. A per-bucket-size pre-check (`BucketTooLargeError`) now escalates the ladder *before* anything is written, instead of after a failed write.

**A second, latent data-loss bug was fixed at the same time.** The old `migrateLegacyItems()` did `set({[bKey]: encodeBucket(itemsForThisAttempt)})`. That overwrote any bucket a *previous partial attempt* had already written, and that attempt had already removed the matching legacy keys. It happens whenever an attempt partly succeeds at one count and a retry, or a later load, runs at another count. Reproduced it: 60 of 300 items were dropped from sync storage (they were still in the local backup). Migration now always merges into existing bucket contents, and the rebalance afterwards regroups everything under a single count.

**How this was reproduced:** a Node mock that enforces Firefox's real rule (total = `JSON.stringify(wholeMap)` bytes, per-item = key + JSON value, 512 keys), seeded with 332 synthetic legacy items padded to 99,833 bytes. The pre-fix build gives *exactly* the user's diagnostics. The fixed build converts everything with ~47 writes. A sweep of 144 cases (150–340 items, 40 to 2,500 bytes of real free space, several seeds) passed with no item loss or duplication and `settings` untouched. The worst case was 59 writes, under Chrome's 120/minute limit. **Model Firefox's serialized-total rule in any future quota mock.** A mock that sums `getBytesInUse` misses this class of bug.

**Storage Diagnostics now also reports** (counts, sizes and timestamps only, never URLs or titles, so users can send it safely): unconverted legacy item count/bytes, the serialized byte total, key count, local backup presence, a log of the last conversion (`lastMigration` in `chrome.storage.local`: bucket counts tried, writes, failed writes, splits, swaps, error), whether the last load error has since been resolved, and the user agent. `markLoadErrorResolved()` stamps `resolvedAt` on the first successful load after an error. Before this, a fixed error stayed on screen and looked current (the user's second report showed the old error next to a healthy 332-item list).
