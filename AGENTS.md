...
- **`src/styles/`** — every component's `*.styles.ts` (Lit `css` template), deliberately kept separate from `src/components/` rather than colocated, since `theme.styles.ts` isn't any one component's style. Each `<component>.styles.ts` here corresponds 1:1 to the component of the same name in `src/components/`.
