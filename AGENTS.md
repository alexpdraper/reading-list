...
- **`src/components/reading-list-app.ts`** — the main list UI: search box, all/unread filter, date/title sort, the add-current-page button, drag-and-drop reordering, the staggered reveal animation on first load, the edit-mode dimming overlay, and the load-error/sync-error banners. Also owns the "leave a review" nag (`maybeGetReviewItem`/`dismissReview`) — a synthetic list item shown once after 6+ saved items.
- **`src/components/reading-list-options.ts`** — the settings page: the default-behavior checkboxes, Export/Import (`downloadJson` is the shared Blob-and-`<a download>` mechanics behind both Export and "Download Local Backup"), the Storage Diagnostics button, "Download Local Backup", and Clear Reading List.
...
- **`src/styles/theme.styles.ts`** — the single source of every color, shadow, and shared typography value (`--rl-bg-color`, `--primary-color`, `--rl-shadow`, etc.), light and dark, plus the box-sizing/`:focus-visible` reset. Composed into every Shadow-DOM component as `static override styles = [theme, reset, ...]`.
- **`src/styles/app.styles.ts`** — `reading-list-app.ts`'s own styles: the app shell and list spacing, the sync-error banner, the editing overlay, plus the header (title, save/settings/sidebar buttons), search input, and filter/sort control styles.
...
**Decision:** `@lit-labs/motion`'s `animate()` directive, keyed off `repeat()`, drives every entry/exit and reorder animation. `animate()` keeps a removed element in the DOM for its own exit animation, so the component never needs to know *why* an item left the list — a local delete, a remote removal and a search filter all take the same path.
