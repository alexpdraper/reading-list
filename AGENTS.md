...
- **`src/styles/`** — every component's `*.styles.ts` (Lit `css` template), deliberately kept separate from `src/components/` rather than colocated, since `theme.styles.ts` isn't any one component's style. Each `<component>.styles.ts` here corresponds 1:1 to the component of the same name in `src/components/`, plus three cross-cutting files composed alongside them: `theme.styles.ts` (below), `reset.styles.ts` (box-sizing reset + the shared `:focus-visible` outline — compose it into *every* component, it used to be duplicated into app/item and simply missing from options), and `animations.styles.ts` (the `slidein`/`slidein-bounce`/`slideout` keyframes and the classes that trigger them — only `reading-list-item.ts` uses it today, split out anyway to mirror the pre-rewrite app's `animations.styl`, which kept the same content in its own file for the same reason: it's a distinct concern from the rest of the item's rules even with one consumer).
